...
import type { DevelopmentCardData } from "@/lib/developments";
import { DevelopmentCard } from "@/components/site/development-card";

interface DevelopmentsGridProps {
  developments: DevelopmentCardData[];
}

export function DevelopmentsGrid({ developments }: DevelopmentsGridProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return developments.filter((d) =>
      [d.name, d.description, d.location, d.slug]
        .filter(Boolean)
