...
**Versión:** 1.8 — 25-Ago-2026
**Estado:** MVP en desarrollo — Auth i18n completo, comunidades en DB (migración 008), ruta /app, developer pages en DB + editor rich text TipTap (migraciones 011–013), broker profile pages + form (migración 014), property upload & management system (form 10 secciones, milestones CRUD, CRUD completo)
...
| Property upload form con 10 secciones (basic info, location, details, pricing, milestones, amenities, images, tags, links, visibility) | Vendedor | ✅ Implementado |
...
- **Server page:** verifica sesión + perfil (`user_profiles`), carga ciudades por operating country, amenities, subcategories, y developments (solo para developers con `developer_id`)
- Renderiza `PropertyForm` con `property={null}` (modo creación)
...
- Renderiza `PropertyForm` con `property={property}` + `milestones` (modo edición)
- **Delete:** botón destructivo en el form, ejecuta `deleteProperty(id)` con ownership check
...
| Sección | Campos |
|---|---|
| **Basic Information** | Title*, Slug (auto, read-only + copy URL), Description (textarea), Property Type (select), Subcategory (select grouped by category), Status (select) |
| **Location** | Country (read-only desde perfil), City (select por operating country), Community, Address |
| **Property Details** | Bedrooms, Bathrooms, Floor, Area (sqft, auto-calc sqm), Balcony checkbox, Garden checkbox |
| **Pricing** | Price*, Currency (select), Deposit %, Deposit Amount (auto-calculated from % + price), Handover Date, Payment Plan Months, Post Handover checkbox |
| **Payment Plan Milestones** | `MilestonesEditor` — CRUD dinámico de hitos |
| **Amenities** | Toggle pills agrupados por categoría (desde `property_amenities` curados) |
| **Images** | Cover Image (ImageUpload a bucket `property-images`), Gallery Images (upload múltiple, hasta 10, con preview y delete) |
| **Tags** | Input + Enter para agregar tags, chips removibles |
| **Links** | Development (select de developments propios, solo para developers) |
| **Visibility** | Active checkbox (controla `is_active`) |
...
- **Delete:** `confirm()` nativo → `deleteProperty()` → redirect a `/app/properties`
...
- Valida: title (requerido, max), slug regex `^[a-z0-9-]+$`, description max (tras sanitizar), property_type, status, currency, price > 0, deposit_percentage 0–100
...
| `/app/properties/new` | Autenticado | Crear propiedad (PropertyForm con 10 secciones) |
...
5. NewPropertyPage carga ciudades del operating_country, amenities, subcategories, y developments (solo developers)
6. PropertyForm muestra 10 secciones: Basic Info, Location, Details, Pricing, Milestones, Amenities, Images, Tags, Links, Visibility
7. Slug se auto-genera desde Title (read-only); sqm se auto-calcula de sqft; deposit_amount se auto-calcula de price × deposit%
8. Sube cover image a property-images bucket; gallery images (hasta 10) al mismo bucket
9. Editar milestones con MilestonesEditor (CRUD dinámico); validación de total ≤ 100%
10. Save → saveProperty() (validación server-side + sanitización description + ownership check) → saveMilestones() → router.refresh()
11. Botón "Edit" en PropertyList → /app/properties/{id}/edit → mismo PropertyForm en modo edición
12. Botón "Delete" en modo edición → confirm() nativo → deleteProperty() (ownership check) → redirect a /app/properties
13. hasChanges compara cada campo; botón Save deshabilitado sin cambios
```
...
42. **El upload y gestión de propiedades (`/app/properties`) está implementado para los 3 roles.** El form PropertyForm tiene 10 secciones, incluye MilestonesEditor para plan de pago, auto-cálculos (sqm↔sqft, deposit_amount↔%), slug auto-generado, y gallery upload al bucket `property-images`. Las server actions (`saveProperty`, `deleteProperty`, `saveMilestones`) incluyen validaciones server-side y ownership checks (`listed_by_id = user.id`). Las descripciones se sanitizan con `sanitizeUserHtml()` antes de persistir./
...
**Restricciones:**
- Las tablas `properties`, `payment_plan_milestones`, `developers`, `developments` y `communities` se leen de Supabase en la UI pública y en el dashboard de vendedores. `lib/mock-properties.ts` fue eliminado. La tabla `developments` aún no tiene página de listado conectada a DB (usa mock data)
...
- Property upload form (`PropertyForm`) con 10 secciones: Basic Info, Location, Details, Pricing, Milestones, Amenities, Images, Tags, Links, Visibility
...
| PropertyForm | Componente de formulario para crear/editar propiedades en el dashboard; 10 secciones (basic info, location, details, pricing, milestones, amenities, images, tags, links, visibility); client component con auto-cálculos y detección de cambios |
